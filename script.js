(function() {
  ...
  function basicValidate(name, email, message) {
    ...
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return t("Please enter a valid email.");
    if (!message || message.length < 10) return t("Message must be at least 10 characters.");
    return null;
  }
  ...
  function buildMailtoLink({name: name, email: email, message: message}) {
  ...
  function updateScrollUI() {
    ...
    if (nav) nav.style.boxShadow = p > .02 ? "0 14px 40px rgba(0,0,0,.30)" : "none";
  }
  updateScrollUI();
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener("submit", async e => {
      ...
      try {
        ...
        setStatus(contactStatus, "error", t("Could not send via EmailJS. Click to open an email draft."));
  ...
  if (hero && orb) {
    ...
    if (!reduce) {
      ...
      function applyParallax(e) {
        ...
        if (circle) {
          circle.style.transform = `translate3d(${(x - .5) * 10}px, ${(y - .5) * 10}px, 0)`;
        }
        if (scan) {
          scan.style.opacity = String(.08 + x * .1);
  ...
  const MESSAGES_AR = {
    ...
    "Could not send via EmailJS. Click to open an email draft.": "تعذر الإرسال عبر EmailJS. اضغط لفتح مسودة بريد إلكتروني.",
  ...
  function applyLanguage(lang) {
    ...
    $$("[data-i18n-ar-placeholder]").forEach(el => {
      ...
      el.setAttribute("placeholder", isAr ? el.dataset.i18nArPlaceholder : el.dataset.i18nEnPlaceholder);
    });
    $$("[data-tooltip-ar]").forEach(el => {
      if (el.dataset.tooltipEn === undefined) {
        el.dataset.tooltipEn = el.getAttribute("data-tooltip") || "";
      }
      el.setAttribute("data-tooltip", isAr ? el.dataset.tooltipAr : el.dataset.tooltipEn);
    });
